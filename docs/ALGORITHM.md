...
2. For a DSpace 7 entity URL or persistent `hdl.handle.net` URL, resolve the
...
3. Otherwise, resolve the input identifier to a DOI string plus optional arXiv metadata.
- `_resolve_identifier()` in `doi2bib3/backend.py`

4. Fetch BibTeX for that DOI (`doi.org` first, Crossref transform fallback).
- `_fetch_bibtex_for_doi()` in `doi2bib3/backend.py`

5. Normalize BibTeX fields and formatting.
...
6. Return normalized BibTeX text.
